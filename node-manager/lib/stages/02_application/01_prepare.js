...
class Child extends Phase {
    ...
    preparePlaybooks(containerVarPaths) {
        ...
        for (const containerVarPath of containerVarPaths) {
            playbooks.push(new common.Playbook(this.playbookPath, this.varPath, ["-i", `${conf.runDir}hosts`, `--key-file=${conf.runDir}${this.infra.infra.aws.ssh_key_name}.pem`, `--extra-vars=@${containerVarPath}`]))
        }
        return playbooks
    }
